...
The Todo collection will contain a [type] with a [string] prototype, the status [completed] and a [boolean] value, plus a value id as a string. Any categories can be configured in the schema it is up to the
