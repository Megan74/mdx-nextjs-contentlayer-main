...
// invoke the schema functionality in the Todo.js file, and gives you the liberty to create your own customized collection with key-pair values.
...
// will export the mongoose model in any files needed, and be called by the controller to be rendered in view as server response
...
The Todo collection will contain a [type] with a string prototype, the status [completed] and a [boolean] value, plus a [userId] as a string. Any properties can be configured in the mongoose schema, it is up to the
